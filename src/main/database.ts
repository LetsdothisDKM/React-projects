...
export function setupIpcHandlers(): void {
  ...
  ipcMain.handle('get-or-create-player', (_, playerName: string) => {
    ...
    let player = getDatabase().prepare('SELECT * FROM players WHERE name = ?').get(playerName)
  ...
  ipcMain.handle('delete-player', (_, playerName: string) => {
    // Check if player exists
    getDatabase().prepare('DELETE FROM players WHERE name = ?').run(playerName)
  })
}
